...
export default function TCPRouterForm() {
  ...
  const { data: existingRouter, isLoading: isLoadingRouter } = useQuery({
    queryKey: ['resources', 'tcp', 'routers', name],
    queryFn: async () => {
      if (!name) return null
      const response = await resourcesApi.get('tcp', 'routers', name)
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      const response = await resourcesApi.create('tcp', 'routers', formData)
      return response.data
    },
    onSuccess: () => {
      notifications.show({
        title: 'Success',
        message: 'TCP router created successfully',
        color: 'green',
      })
      queryClient.invalidateQueries({ queryKey: ['resources', 'tcp', 'routers'] })
      navigate('/routers')
    },
    onError: (error: any) => {
      notifications.show({
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      const response = await resourcesApi.update('tcp', 'routers', name, {
    ...
    onSuccess: () => {
      notifications.show({
        title: 'Success',
        message: 'TCP router updated successfully',
        color: 'green',
      })
      queryClient.invalidateQueries({ queryKey: ['resources', 'tcp', 'routers'] })
      navigate('/routers')
      ...
      <Stack gap="lg">
        ...
        <Group justify="space-between">
          <Group>
            ...
            <div>
              <Title order={2}>{isEditMode ? 'Edit' : 'Create'} TCP Router</Title>
              <Text c="dimmed" size="sm">
                {isEditMode ? `Update router: ${formData.name}` : 'Create a new TCP router'}
              </Text>
            </div>
          </Group>
          <Button
