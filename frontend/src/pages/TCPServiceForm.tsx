...
export default function TCPServiceForm() {
  ...
  const { data: existingService, isLoading: isLoadingService } = useQuery({
    queryKey: ['resources', 'tcp', 'services', name],
    queryFn: async () => {
      if (!name) return null
      const response = await resourcesApi.get('tcp', 'services', name)
  ...
  useEffect(() => {
    if (existingService) {
      // Detect service subtype from config and unwrap
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      const response = await resourcesApi.create('tcp', 'services', payload)
      return response.data
    },
    onSuccess: () => {
      notifications.show({
        title: 'Success',
        message: 'TCP service created successfully',
        color: 'green',
      })
      queryClient.invalidateQueries({ queryKey: ['resources', 'tcp', 'services'] })
      navigate('/services')
    },
    onError: (error: any) => {
      notifications.show({
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      // Wrap config under the subtype key
      const response = await resourcesApi.update('tcp', 'services', name, {
    ...
    onSuccess: () => {
      notifications.show({
        title: 'Success',
        message: 'TCP service updated successfully',
        color: 'green',
      })
      queryClient.invalidateQueries({ queryKey: ['resources', 'tcp', 'services'] })
      navigate('/services')
    ...
    <Container size="md">
      <Stack gap="lg">
        ...
        <Group justify="space-between">
          <Group>
            <IconNetwork size={32} stroke={1.5} color="#00aec1" />
            <div>
              <Group gap="xs">
                <Title order={2}>{isEditMode ? 'Edit' : 'Create'} TCP Service</Title>
                <Badge color="green" variant="light" size="lg">
                  {subtypeLabels[formData.subtype]}
                </Badge>
              </Group>
              <Text c="dimmed" size="sm">
                {isEditMode ? `Update service: ${formData.name}` : `Create a new TCP ${subtypeLabels[formData.subtype].toLowerCase()} service`}
              </Text>
            </div>
          </Group>
          <Button
        ...
        <form onSubmit={handleSubmit}>
          <Stack gap="lg">
            ...
            <Card shadow="sm" radius="md" withBorder>
              <ServiceSchemaForm
                protocol="tcp"
                subtype={formData.subtype as 'loadBalancer' | 'weighted'}
