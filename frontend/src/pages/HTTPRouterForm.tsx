...
import { IconRouter, IconDeviceFloppy, IconArrowLeft } from '@tabler/icons-react'
import { resourcesApi } from '@/lib/api'
import { SchemaForm } from '@/components/SchemaForm'

export default function HTTPRouterForm() {
  ...
  const { data: existingRouter, isLoading: isLoadingRouter } = useQuery({
    queryKey: ['resources', 'http', 'routers', name],
    queryFn: async () => {
      if (!name) return null
      const response = await resourcesApi.get('http', 'routers', name)
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      const response = await resourcesApi.create('http', 'routers', formData)
      return response.data
    },
    onSuccess: () => {
      notifications.show({
        title: 'Success',
        message: 'HTTP router created successfully',
        color: 'green',
      })
      queryClient.invalidateQueries({ queryKey: ['resources', 'http', 'routers'] })
      navigate('/routers')
    },
    onError: (error: any) => {
      notifications.show({
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      const response = await resourcesApi.update('http', 'routers', name, {
    ...
    onSuccess: () => {
      notifications.show({
        title: 'Success',
        message: 'HTTP router updated successfully',
        color: 'green',
      })
      queryClient.invalidateQueries({ queryKey: ['resources', 'http', 'routers'] })
      navigate('/routers')
      ...
      <Stack gap="lg">
        ...
        <Group justify="space-between">
          <Group>
            ...
            <div>
              <Title order={2}>{isEditMode ? 'Edit' : 'Create'} HTTP Router</Title>
              <Text c="dimmed" size="sm">
                {isEditMode ? `Update router: ${formData.name}` : 'Create a new HTTP router'}
              </Text>
            </div>
          </Group>
          <Button
